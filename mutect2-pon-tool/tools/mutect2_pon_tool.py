...
def pon(uuid, bam_path, thread_count, reference_fasta_path, cosmic_path, intervals_path, known_snp_vcf_path, engine, logger):
  ...
  out_pon_vcf = os.path.join(step_dir, bam_base) + '_pon.vcf'
  logger.info('out_pon_vcf=%s' % out_pon_vcf)
  if pipe_util.already_step(step_dir, bam_name + '_MuTect2_PON', logger):
    logger.info('already completed step `MuTect2 Panel Of Normal calling` of: %s' % bam_path)
  else:
    ...
    unique_key_dict = {'uuid': uuid, 'bam_path': bam_path, 'thread_count': thread_count, 'out_pon_vcf': out_pon_vcf}
    df_util.save_df_to_sqlalchemy(df, unique_key_dict, table_name, engine, logger)
    pipe_util.create_already_step(step_dir, bam_name + '_MuTect2_PON', logger)
    logger.info('completed running step `MuTect2 Panel Of Normal calling` of: %s' % bam_path)
  return
